...
export function getDefaultColumnData(
  ...
  const foreignTableSchema =
    columnSchema.foreignKey && databaseSchema.tables.find((t) => t.name === columnSchema.foreignKey?.table)
  ...
  return {
    ...
    label: columnSchema.foreignKey
      ? getLabelFromForeignKeyColumnName(columnSchema.name)
      : transformToReadable(columnSchema.name),
    hiddenInTable: false,
...
export function removeDefaultColumnData(
  data: TablePageConfigColumn,
  defaultColumnData: TablePageConfigColumn
): TablePageConfigColumn | undefined {
  ...
  return Object.keys(finalData).length === 0
    ? undefined
    : {
        ...finalData,
        column: data.column,
...
export function getTableData(params: {
  ...
  // Table schema
  const tableSchema = tablePageConfig.table
    ? databaseSchema?.tables.find((t) => t.name === tablePageConfig.table)
    : undefined
  ...
  const primaryKeyColumn =
    tablePageConfig?.primaryKeyColumn ?? tableSchema?.columns.filter((c) => c.primaryKey)[0]?.name
  ...
  const columns = tableSchema
    ? tableSchema.columns.map((c) => {
        ...
        const defaultColumnData = getDefaultColumnData(tablePageConfig.table!, c.name, databaseSchema!)
        ...
        return {
          ...
          relationshipPreviewColumns:
            column?.relationshipPreviewColumns ?? defaultColumnData.relationshipPreviewColumns,
  ...
  // Relationships
  const autoDetectedRelationships =
    (databaseSchema && getAllPossibleRelationships(tablePageConfig, databaseSchema)) ?? []
